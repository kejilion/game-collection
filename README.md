...
| **战斗操作** | 方向键移动，`A` 普攻，`1~5` 技能区（每职业 1 个技能，预留扩展） |
...
- **新增技能**：在 `server/config.js` 的对应职业 `skills` 数组里加一项，并在 `world.js` 的 `doSkill()` 中实现效果；客户端技能栏会自动点亮 `2~5` 槽位。
