...
export class Renderer {
  constructor(canvas) {
    ...
    this.particles = [];
    this.phases = new Map(); // id ï¿?run-cycle phase
    ...
    // twinkling stars ï¿?only shown in the dark lower reaches
    ...
    // slow aurora ribbons ï¿?dreamy near the bottom, faint up high
    this.aurora = [
    ...
    this._p = 0; // altitude progress 0(bottom)ï¿?(top), set each frame
    // backing-store scale: logical 1600Ã—900 units ï¿?device pixels (set by resize)
    this.scale = (canvas.width || VIEW_W) / VIEW_W;
  }

  /**
   * Size the backing store to the displayed CSS size Ã— devicePixelRatio so the
   * 16:9 logical space stays crisp on HiDPI screens.  The frame is locked to
   * 16:9 by the layout, so X and Y share one uniform scale ï¿?no distortion.
   */
  ...
  addFx(fx) {
    ...
    switch (fx.t) {
      ...
      case 'mdeath':
        ...
        break;
      case 'death': {
        // å¤§çˆ†ç‚¸ï¼šå†°æ™¶ç¢Žç‰‡ + æš–è‰²ç«æ˜Ÿ + å†²å‡»æ³¢çŽ¯ + é£˜å­—
        ...
        P.push(dmgText(fx.x, fx.y - PLAYER_H, 'ðŸ”¥', '#ff6a3c', 26));
  ...
  draw(scene, dt) {
    const ctx = this.ctx;
    // map logical 1600Ã—900 ï¿?backing pixels (replaces the implicit identity
    // transform); everything below keeps drawing in clean 1600Ã—900 units.
  ...
  _background(ctx, cam) {
    ...
    const p = clamp(1 - cam / maxCam, 0, 1); // 0 = bottom (dark) ï¿?1 = top (bright)
    this._p = p;
    const ease = p * p * (3 - 2 * p); // smoothstep

    // sky: deep dreamy night ï¿?bright hopeful dawn
    ...
    // aurora ribbons ï¿?strongest in the dreamy depths, a faint shimmer up top
    ...
    // parallax peaks ï¿?dark silhouettes below, bright snowy ridges up high
  ...
  _peaks(ctx, off, color, base, height, ease = 1) {
    ...
    ctx.closePath();
    ctx.fill();
    // snow caps ï¿?bright at the hopeful summit, dim in the dark depths
  ...
  _platform(ctx, r) {
    ...
    const { x, y, w, h } = r;
    if (r.type === Tile.ICE) {
      // a breakable ice brick ï¿?a row of these forms a wall to bust through
      ...
      // wood lift ï¿?square bricks (every ledge reads as a row of square blocks)
      ...
      // acceleration strip ï¿?square bricks under flowing motion chevrons
      this._brickCols(x, w, h, (cx, cw) => {
        roundRect(ctx, cx + 0.5, y, cw - 1, h, 6);
    ...
    } else { // SOLID rock ï¿?unbreakable square bricks, route around them
      this._brickCols(x, w, h, (cx, cw) => {
  ...
  _item(ctx, it) {
    ...
    const icon = it.kind === ItemKind.FIRE ? 'ðŸ”¥' : it.kind === ItemKind.HEAL ? 'â¤ï¸' : 'ðŸ¦…';
  ...
  _player(ctx, e, look, name, dt, isLocal) {
    ...
    // ---- æ­»äº¡å€’åœ°ï¼šå•ç‹¬ç»˜åˆ¶ï¼ŒåŽç»­ä¸å†ç”»è¡€ï¿?buff ----
    if (e.dead > 0) {
      ctx.save();
    ...
    let bx = e.x + 18;
    if (e.fb) { ctx.font = '20px system-ui'; ctx.textAlign = 'left'; ctx.fillText('ðŸ”¥', bx, topY + 4); bx += 24; }
    if (e.jb) { ctx.font = '20px system-ui'; ctx.textAlign = 'left'; ctx.fillText('ðŸŸ¢', bx, topY + 4); }
  }
  ...
  _plane(ctx, plane, local) {
    ...
    // pickup glow zone (the rope's grab reach) ¡ª travels with the plane
    ctx.save();
    ctx.setLineDash([6, 5]);
